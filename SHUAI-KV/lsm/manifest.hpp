...
/**
 * @brief Manifest 类 - SST 文件元数据管理器
 *
 * Manifest 采用 Copy-on-Write 模式，每次修改都创建新版本：
 * - 读操作：读取最新版本的 Manifest
 * - 写操作：创建新版本，复制旧版本数据
 *
 * 这种设计保证了读操作无需加锁，提高了读取性能。
 */
class Manifest {
public:
    /**
...
public:
    ...
    void SizeTieredCompaction(size_t level, int id) {
        ...
        // 生成新的 SST 文件
        auto new_sst_ptr = std::make_shared<SST>(entries, id);
    ...
    void SizeTieredCompaction(int id) {
        for (int i = 0; i < static_cast<int>(levels_.size()) && i < max_level_size_; ++i) {
