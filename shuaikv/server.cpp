#include "shuaikv/raft/service.hpp"
#include "shuaikv/raft/config.hpp"
...
void PrintUsage(const char* prog) {
    std::cout << "Usage: " << prog << " [options]\n"
              ...
              << "  -l, --log <file>   Log file path (default: shuaikv.log)\n"
              << "  -P, --pid <file>   PID file path (default: shuaikv.pid)\n"
...
void RunServer() {
    shuaikv::ResourceManager::instance().InitDb();
    shuaikv::ResourceManager::instance().InitPod();

    auto local_addr = shuaikv::ResourceManager::instance().config_manager().local_address();
    std::string server_addr = local_addr.ip() + ":" + std::to_string(local_addr.port());

    shuaikv::EasyKvServiceServiceImpl service;
    ...
    std::cout << "Shuai-KV server listening on " << server_addr << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    std::string log_file = "shuaikv.log";
    std::string pid_file = "shuaikv.pid";
    ...
    while ((opt = getopt_long(argc, argv, "dc:l:P:hv", long_options, nullptr)) != -1) {
        switch (opt) {
            ...
            case 'h':
                ...
                return 0;
            case 'v':
                std::cout << "Shuai-KV version 0.1.0" << std::endl;
                return 0;
            default:
                PrintUsage(argv[0]);
                return 1;
    ...
    shuaikv::ResourceManager::instance().Close();
