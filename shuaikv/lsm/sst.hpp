...
private:
    ...
    shuaikv::common::BloomFilter bloom_filter_;
...
public:
    ...
    bool PrefetchDataBlock(size_t block_index) {
        ...
        // 检查是否已缓存
        if (block_cache_->Get(id_, block_index) != nullptr) {
            return true;  // 已缓存
        }
        ...
        size_t block_size = *reinterpret_cast<size_t*>(data_ + index);
        index += sizeof(size_t);

        // 检查 block 大小是否有效
        if (block_size == 0 || index + block_size > file_size_) {
            return false;
        }

        // 复制数据到缓存
        std::vector<char> block_data(data_ + index, data_ + index + block_size);
